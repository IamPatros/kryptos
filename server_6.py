...
def supa_get(table, params=None):
    r = requests.get(f"{SUPABASE_URL}/rest/v1/{table}", headers=sh(), params=params, timeout=10)
    r.raise_for_status(); return r.json()

def supa_post(table, data):
    r = requests.post(f"{SUPABASE_URL}/rest/v1/{table}", headers=sh(), json=data, timeout=10)
    r.raise_for_status(); return r.json()

def supa_patch(table, filter_str, data):
    r = requests.patch(f"{SUPABASE_URL}/rest/v1/{table}?{filter_str}", headers=sh(), json=data, timeout=10)
    r.raise_for_status(); return r.json()
