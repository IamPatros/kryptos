...
def resolve_pending():
    ...
    c = conn.cursor()
    now_ms = int(time.time() * 1000)
    c.execute("SELECT id, window_start, window_end, direction, open_price FROM predictions WHERE result='pending'")
    rows = c.fetchall()
    for row in rows:
        pid, ws, we, direction, open_price = row
        if now_ms < we:
        ...
        try:
            ...
            c.execute("""
                UPDATE predictions SET result=?, close_price=?, price_diff=?
                WHERE id=?
            """, (result, close_price, price_diff, pid))
            conn.commit()
...
def save_prediction(result, ws_s):
    ...
    c.execute("""
        ...
        (window_start, window_end, signal_time, direction, confidence, score, open_price, factors, market_data)
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (
        ws_ms, we_ms, signal_time,
        result["direction"], result["confidence"], result["score"],
        result["open_price"],
    ...
    conn.commit()
    conn.close()
    print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] 📊 Prediction saved: {result['direction']} | Score {result['score']}/20 | Conf {result['confidence']}%")
